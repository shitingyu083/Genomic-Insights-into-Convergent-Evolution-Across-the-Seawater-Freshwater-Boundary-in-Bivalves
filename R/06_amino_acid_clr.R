...
run_amino_acid_clr <- function(paths, outdir) {
  ...
  tests <- do.call(rbind, lapply(split(aa, list(aa$protein_set, aa$amino_acid), drop = TRUE), function(y) {
    tmp <- unique(y[, c("species_id", "habitat_binary", "clr")])
    names(tmp)[names(tmp) == "clr"] <- "clr_response"
    pgls <- fit_pgls_habitat(tmp, "clr_response", tree, species_col = "species_id")
    data.frame(
      ...
      estimate_freshwater_minus_seawater_group = pgls$estimate_freshwater_minus_seawater_group,
